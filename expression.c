...
precendencyOperation getOperationFromTable(precedencyInput onStack, precedencyInput onInput) {
    if (onStack == EXPR_NON_TERMINAL || onStack == EXPR_HANDLE || onStack == EXPR_ERROR ||
        onInput == EXPR_NON_TERMINAL || onInput == EXPR_HANDLE || onInput == EXPR_ERROR) {
            return N;
    }
    else {
...
bool reduceDyadicOperation(TStack *stack, StackItem **possibleRule) {   //(a+b, a*b, (a), ...)
    if (possibleRule[0]->item == EXPR_R_BRACKET && possibleRule[1]->item == EXPR_NON_TERMINAL && possibleRule[2]->item == EXPR_L_BRACKET) {
    ...
    else if (possibleRule[0]->item == EXPR_NON_TERMINAL && possibleRule[1]->item == EXPR_PLUS_MINUS && possibleRule[2]->item == EXPR_NON_TERMINAL) {
    ...
    else if (possibleRule[0]->item == EXPR_NON_TERMINAL && possibleRule[1]->item == EXPR_MULDIV && possibleRule[2]->item == EXPR_NON_TERMINAL) {
        insertAfter(possibleRule[1]->tok);
        deleteNearestItem(stack, EXPR_NON_TERMINAL);
    ...
    else if (possibleRule[0]->item == EXPR_NON_TERMINAL && possibleRule[1]->item == EXPR_REL && possibleRule[2]->item == EXPR_NON_TERMINAL) {
        insertAfter(possibleRule[1]->tok);
        deleteNearestItem(stack, EXPR_NON_TERMINAL);
    ...
    else if (possibleRule[0]->item == EXPR_NON_TERMINAL && possibleRule[1]->item == EXPR_CONCAT && possibleRule[2]->item == EXPR_NON_TERMINAL) {
        insertAfter(possibleRule[1]->tok);
        deleteNearestItem(stack, EXPR_NON_TERMINAL);
...
bool reduceIdentifier(TStack *stack, StackItem **possibleRule) {
    if (possibleRule[0]->item == EXPR_ID || possibleRule[0]->item == EXPR_NON_TERMINAL) {
...
precedencyAnalysisReturn parseExpression(tlist *tokens) {
    ...
    while (!endOfExpression(tokens->t)) {
        ...
        if (op == S) {
            ...
            if (input != EXPR_R_BRACKET) {
                ...
                while (1) {
                    if (stack.first->next->item == EXPR_L_BRACKET && stack.first->item == EXPR_NON_TERMINAL) {
                        break;
                    }
                    if (!reduce(&stack)) {
            ...
            if ((input == EXPR_ID && opOnStack == EXPR_ID) || (input == EXPR_ID && opOnStack == EXPR_R_BRACKET)) {
                break;
            }
            error = true;
        }
    ...
    while (!error) {
        if (stack.first->item == EXPR_NON_TERMINAL && stack.first->next->item == EXPR_DOLLAR) {
            break;
        }
        if (!reduce(&stack)) {
            error = true;
